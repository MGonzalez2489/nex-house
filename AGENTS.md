...
- **Web test suite (now green):** the full web unit suite passes (71 suites / 304 tests, including `features/auth`). The `should create` specs for `SidebarItem`, `Sidebar`, `NavBar`, `FormFeedback` and `UnitsTable` were missing required inputs (`NG0950`) and now set them via `fixture.componentRef.setInput(...)` with `provideRouter([])` where the template uses router links. `form-validation-error.spec.ts` had an intermittent `SIGSEGV` worker crash under full-parallel runs — environment flake, passes repeatedly; don't chase it unless it recurs.
